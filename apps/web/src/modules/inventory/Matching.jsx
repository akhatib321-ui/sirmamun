...
export default function Matching({ initialReportId, onRunReorderNow }) {
  ...
  const reportOptions = useMemo(() => {
    return reports.map((r) => ({ id: r.id, label: `${new Date(r.reportDate).toLocaleDateString()} (${r._count?.items || 0} items)` }));
  }, [reports]);
    ...
    <div style={{ padding: 20, height: 'calc(100vh - 88px)', overflow: 'auto' }}>
      <div style={{ ...ui.card, padding: 16, marginBottom: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 10, marginTop: 14 }}>
          ...
          <button style={{ ...ui.button, background: '#16121a', color: '#fff' }} onClick={handleRunReorderNow}>
            Run Reorder Now
          </button>
        </div>
      </div>
      ...
      {loading && <div style={{ ...ui.card, padding: 14 }}>Loading unmatched items...</div>}
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              {unmatched.map((item) => (
                <div key={item.id} style={{ border: '1px solid #efe5d8', borderRadius: 12, padding: 12 }}>
                  ...
                  <div style={{ color: tokens.colors.muted, fontSize: 13, marginBottom: 8 }}>Qty sold: {item.qtySold}</div>
