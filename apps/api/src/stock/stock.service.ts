...
export class StockService {
  ...
  // ── Add stock record (item at a location) ──────────────────
  ...
  // ── Adjust a single stock row ───────────────────────────────
  async adjust(stockId: string, newQty: number, note?: string) {
  ...
  async transfer(iid: string, fromLid: string, toLid: string, qty: number) {
    ...
    if (toRow) {
      toRow.qty += qty;
      await this.stockRepo.save(toRow);
    } else {
      toRow = await this.stockRepo.save(
        this.stockRepo.create({ iid, lid: toLid, qty }),
      );
    }
  ...
  // ── Batch stock count (save all at once) ───────────────────
  async batchAdjust(changes: { stockId: string; qty: number }[]) {
    const results = [];
    for (const { stockId, qty } of changes) {
      results.push(await this.adjust(stockId, qty, 'stock count'));
    }
    return results;
  }
  ...
  async bulkImport(rows: {
    itemName: string; uom: string; desc?: string;
    supplier?: string; lowAt?: number;
    locationName: string; qty: number;
  }[]) {
    let created = 0;
    for (const row of rows) {
      ...
      if (!loc) {
        loc = await this.locationRepo.save(
          this.locationRepo.create({ name: row.locationName }),
        );
      }
      ...
      if (!item) {
        item = await this.itemRepo.save(this.itemRepo.create({
          name: row.itemName,
          uom: row.uom,
          desc: row.desc ?? '',
